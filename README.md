...
分數上限 30；平台端 8 分 App 提醒、15 暫停新單（哈欠 +3 只是把分數往上推一點，真正跨線靠的是持續閉眼；DMS 的嘴巴線 0.3 下大笑也會算成哈欠）；**恢復要同時滿足「已休息至少 60 秒」與「分數降到 8 以下」**（`server/config.py`，`--pause` `--resume` `--min-rest`）。沒有可信偵測訊號（裝置未連線、訊號中斷、抓不到臉、攝影機故障）的騎手，App 上也不會收到新訂單。臉轉向側面超過 25°、或偵測不到臉時，分數凍結（不加也不減）。
