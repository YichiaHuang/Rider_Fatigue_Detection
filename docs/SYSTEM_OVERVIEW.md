...
- **為什麼心率加分上限只有 6（低於提醒線 8）**：真實道路上單靠 HRV 的三分類準確率只有 61%（Persson 2019，86 位駕駛）；系統性回顧的準確率從 44% 到 100% 都有（Lu 2022）。所以**單靠心率永遠不會提醒騎手或暫停派單**。
...
| 心率疲勞指標（多模態） | 個人基準 300 秒；近 120 秒心率 ≤ −8% **且** RMSSD ≥ +25%，持續 120 秒後每 30 秒 +1，**上限 6**（低於提醒線 8） |
