...
class FaceMesher:
    ...
    def __init__(self, model_path, delegate_path):
        ...
        # Match outputs by element count rather than op name: different
        # TFLite converter versions rename the conv2d output nodes.
        landmark_idx, score_idx = None, None
        for output in self.interpreter.get_output_details():
            if np.prod(output['shape']) == self.FACE_KEY_NUM * 3:
                landmark_idx = output['index']
            else:
                score_idx = output['index']
        self.outputs_idx = {'landmark': landmark_idx, 'score': score_idx}
