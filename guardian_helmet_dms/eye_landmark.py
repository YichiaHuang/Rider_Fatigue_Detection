...
class EyeMesher:
    ...
    def __init__(self, model_path, delegate_path):
        ...
        # Match outputs by name substring, tolerant of the trailing ":0"
        # that some TFLite converter versions drop.
        eye_idx, iris_idx = None, None
        for output in self.interpreter.get_output_details():
            if 'eye' in output['name']:
                eye_idx = output['index']
            elif 'iris' in output['name']:
                iris_idx = output['index']
        self.outputs_idx = {'eye': eye_idx, 'iris': iris_idx}
